	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		else
		{
			for (int j = 0, nj = (int)p->vertCount; j < nj; ++j)
			{
				...
				float v0[3] = { -_v1[0], _v1[1], _v1[2] };
				float v1[3] = { -_v0[0], _v0[1], _v0[2] };
				...
				for (int k = 0; k < pd->triCount; ++k)
				{
					...
					for (int n = 0; n < 3; ++n)
					{
						...
						else
						{
							...
							tv[m][1] = tile->detailVerts[(pd->vertBase + (t[m] - p->vertCount)) * 3 + 1];
							tv[m][2] = tile->detailVerts[(pd->vertBase + (t[m] - p->vertCount)) * 3 + 2];
						}
					}
					for (int m = 0, n = 2; m < 3; n = m++)
					{
